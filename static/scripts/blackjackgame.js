//BlackJack 
let blackjackGame = {
    'player': { 'scoreSpan': '#your-blackjack-result', 'div': '#your-box', 'score': 0 },
...
function blackjackHit() { if (blackjackGame['isStand'] === false) { let card = randomCard(); showCard(card, PLAYER); updateScore(card, PLAYER); showScore(PLAYER); } }

//Generate cards
function randomCard() { let randomIndex = Math.floor(Math.random() * 52); return blackjackGame['cards'][randomIndex]; }

function showCard(card, activePlayer) {
    if (activePlayer['score'] <= 21) {
        let cardImage = document.createElement('img');
        cardImage.src = `static/img/BJ/CARDS/${card}.png`;
...
function updateScore(card, activePlayer) {
    if (card === 'Ab' || card === 'Ac' || card === 'Ap' || card === 'Ak') {
        ...
        if (activePlayer['score'] + blackjackGame['cardsMap'][card][1] <= 21) { activePlayer['score'] += blackjackGame['cardsMap'][card][1]; }
...
function computeWinner() {
    ...
    if (PLAYER['score'] <= 21) {
        ...
        if (PLAYER['score'] > DEALER['score'] || DEALER['score'] > 21) { blackjackGame['wins']++; winner = PLAYER; }
        else if (PLAYER_1['score'] < DEALER['score']) { blackjackGame['losses']++; winner = DEALER; }
        else if (PLAYER['score'] === DEALER['score']) { blackjackGame['draws']++; }
    }

    // condition: when player busts but dealer dosen't 
    else if (PLAYER['score'] > 21 && DEALER['score'] <= 21) { blackjackGame['losses']++; winner = DEALER; }

    // condition: when player AND the dealer busts
    else if (PLAYER['score'] > 21 && DEALER['score'] > 21) { blackjackGame['draws']++; }
